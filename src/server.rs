    ...
    Id: Eq + Hash,
{
    ...
    AddNonVotingMembers(HashSet<Id>),
    ...
    Reconfiguration(ClusterConfiguration<Id>),
...
type EventReceiver<S, T, Id> = mpsc::UnboundedReceiver<Event<S, T, Id>>;
type EventSender<S, T, Id> = mpsc::UnboundedSender<Event<S, T, Id>>;

/// These are the various requests the host can make of the Raft server.
pub enum Command<S, T, Id>
where
    Id: Eq + Hash,
{
...
impl<S, T, Id> Debug for Command<S, T, Id>
where
    S: Debug,
    T: Debug,
    Id: Debug + Eq + Hash,
...
type CommandReceiver<S, T, Id> = mpsc::UnboundedReceiver<Command<S, T, Id>>;
type CommandSender<S, T, Id> = mpsc::UnboundedSender<Command<S, T, Id>>;
...
pub struct WorkItem<S, T, Id>
where
    Id: Eq + Hash,
{
    content: WorkItemContent<S, T, Id>,
    #[cfg(test)]
    ack: Option<TimeoutArg>,
}

#[derive(Debug)]
pub enum WorkItemContent<S, T, Id>
where
    Id: Eq + Hash,
{
    #[cfg(test)]
...
pub type WorkItemFuture<S, T, Id> = BoxFuture<'static, WorkItem<S, T, Id>>;

async fn await_cancellation(cancel: oneshot::Receiver<()>) -> bool {
    cancel.await.is_ok()
}

async fn await_cancellable_timeout<S, T, Id>(
    work_item_content: WorkItemContent<S, T, Id>,
...
fn make_cancellable_timeout_future<S, T, Id>(
    work_item_content: WorkItemContent<S, T, Id>,
    duration: Duration,
    #[cfg(test)] scheduled_event: ScheduledEvent<Id>,
    #[cfg(test)] scheduler: &Scheduler<Id>,
) -> (WorkItemFuture<S, T, Id>, oneshot::Sender<()>)
where
    S: 'static + Debug + Send,
    T: 'static + Debug + Send,
    Id: 'static + Debug + Eq + Hash + Send,
{
...
pub struct Server<S, T, Id>
where
    Id: Eq + Hash,
{
    command_sender: CommandSender<S, T, Id>,
    event_receiver: EventReceiver<S, T, Id>,
    #[cfg(test)]
    scheduled_event_receiver: Option<ScheduledEventReceiver<Id>>,
    thread_join_handle: Option<thread::JoinHandle<()>>,
}

impl<S, T, Id> Server<S, T, Id>
where
    Id: Eq + Hash,
{
    ...
    where
        ...
        Id: 'static + Copy + Debug + Display + Ord + Send + Hash,
    {
    ...
    pub fn take_scheduled_event_receiver(
        &mut self
    ) -> Option<ScheduledEventReceiver<Id>> {
        self.scheduled_event_receiver.take()
    }
}

impl<S, T, Id> Drop for Server<S, T, Id>
where
    Id: Eq + Hash,
{
    fn drop(&mut self) {
...
impl<S, T, Id> Stream for Server<S, T, Id>
where
    T: Debug,
    Id: Eq + Hash,
{
    type Item = Event<S, T, Id>;
...
impl<S, T, Id> Sink<Command<S, T, Id>> for Server<S, T, Id>
where
    Id: Eq + Hash,
{
    type Error = ();
